...
- **Core Authentication:** Clients authenticate to Cores using bearer tokens. Tokens are generated by the Core on setup and can be rotated. For multi-user scenarios (team use), each user has their own token with defined scopes.
- **Maestro Authentication:** Cores authenticate to Maestro using registration tokens. Clients authenticate to Maestro using user tokens. Maestro can optionally integrate with OAuth providers (GitHub, Google) for web client login.
- **Messaging Authentication:** WhatsApp and Telegram accounts are linked during Maestro setup. Messages are only processed from verified chat IDs to prevent unauthorized access.

### 12.2 Transport Security

All WebSocket connections use TLS (WSS). For LAN connections, Cores generate self-signed certificates on first run. For WAN connections through Maestro relay, Maestro handles TLS termination. Certificate pinning is supported for high-security environments.

### 12.3 Authorization

Operations are authorized using a scope-based model. Each token carries a set of scopes (`read:files`, `write:files`, `exec:terminal`, `admin:workspace`, etc.). The Core validates scopes on every operation. This enables read-only access patterns (e.g., a monitoring dashboard) or restricted access (e.g., mobile client can chat and approve but cannot execute terminal commands).
