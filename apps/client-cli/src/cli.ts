...
program
  .name('condrix')
  .description('Condrix CLI — Distributed AI Agent Orchestration')
  .version('0.1.0');

program
  .command('connect')
  .description('Connect to a Condrix Core')
  .option('-u, --url <url>', 'Core WebSocket URL', 'ws://localhost:9100')
  .option('-t, --token <token>', 'Authentication token')
  .action((_options) => {
    // TODO: Initialize Ink app, connect to Core
    console.log('Condrix CLI — connecting...');
  });
...
program
  ...
  .action(() => {
    ...
    console.log('Condrix Status — not yet implemented');
  });
