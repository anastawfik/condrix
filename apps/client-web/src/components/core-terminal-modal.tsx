...
export function CoreTerminalModal({ coreId, coreName, open, onClose }: CoreTerminalModalProps) {
  ...
  useEffect(() => {
    ...
    multiCoreStore
      .getState()
      .requestOnCore<{ id: string; title: string }>(coreId, 'core', 'terminal.create', {
        cols: 120,
        rows: 30,
      })
  ...
  const handleClose = useCallback(() => {
    if (terminalId) {
      multiCoreStore
        .getState()
        .requestOnCore(coreId, 'terminal', 'close', { terminalId })
        .catch(() => {});
      setTerminalId(null);
    }
    setError(null);
  ...
  const handleData = useCallback(
    (tid: string, data: string) => {
      multiCoreStore
        .getState()
        .requestOnCore(coreId, 'terminal', 'write', { terminalId: tid, data })
        .catch(() => {});
    },
    [coreId],
  );
  const handleResize = useCallback(
    (tid: string, cols: number, rows: number) => {
      multiCoreStore
        .getState()
        .requestOnCore(coreId, 'terminal', 'resize', { terminalId: tid, cols, rows })
        .catch(() => {});
    },
    [coreId],
  );
  ...
  useEffect(() => {
    ...
    const unsub = multiCoreStore
      .getState()
      .subscribeOnCore(coreId, 'terminal:output', (event: MessageEnvelope) => {
        ...
        if (listeners) {
          for (const l of listeners) l(payload.data);
        }
      });
    return unsub;
  }, [coreId, open]);
  ...
  const handleOutput = useCallback((tid: string, listener: (data: string) => void) => {
    ...
    return () => {
      outputListeners.current.get(tid)?.delete(listener);
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <DialogContent
          className="max-w-4xl h-[500px] p-0 gap-0 flex flex-col"
