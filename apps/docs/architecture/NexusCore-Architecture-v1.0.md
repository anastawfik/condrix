...
Condrix supports two methods for authenticating with AI providers:
...
4. Browser shows an auth code — user pastes it back into the Condrix UI
...
OAuth tokens (`sk-ant-oat01-*`) are scoped to Claude Code's client. Anthropic's backend rejects direct API calls with these tokens for premium models (Sonnet, Opus). Condrix spawns the `claude` CLI as a subprocess (`claude -p --verbose --output-format stream-json --include-partial-messages`) which handles the API call internally with the correct request shape.

**Token lifecycle:**
- `ClaudeAuthManager` service monitors token expiry every 5 minutes
- Tokens are proactively refreshed 30 minutes before expiry (configurable via `CONDRIX_OAUTH_REFRESH_BUFFER_MS`)
...
- Core binds to `127.0.0.1` by default — set `CONDRIX_CORE_HOST=0.0.0.0` for network access.
